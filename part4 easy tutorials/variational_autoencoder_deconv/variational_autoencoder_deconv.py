...
def vae_neural_network(img, args):
    ...
    for i in range(2):
        filters *= 2
        x = fluid.layers.conv2d(input=x, num_filters=filters,
                                filter_size=kernel_size, stride=2, act="relu", padding=1)

    # shape info needed to build decoder model
    shape = x.shape

    # generate latent vector Q(z|X)
    x = fluid.layers.flatten(x=x)
    x = fluid.layers.fc(input=x, size=16, act='relu')
    ...
    # build decoder model
    x = fluid.layers.fc(input=z, size=shape[1] * shape[2] * shape[3], act='relu')
    x = fluid.layers.reshape(x, shape=shape)

    x = fluid.layers.conv2d_transpose(input=x, num_filters=filters,
                                          filter_size=kernel_size, stride=2, act='relu')
    filters //= 2
    ...
    outputs = fluid.layers.conv2d_transpose(input=x, num_filters=1,
                                          filter_size=kernel_size, act='sigmoid', padding=1)
    outputs = fluid.layers.crop(outputs, shape=img.shape)
