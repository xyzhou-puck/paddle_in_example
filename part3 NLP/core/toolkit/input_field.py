...
class InputField(object):
    ...
    def __add__(self, input_slot):
        ...
        if isinstance(input_slot, dict):
            ...
            lod_level = input_slot["lod_level"] if "lod_level" in input_slot else 0
    ...
    def build(self, build_pyreader = False, capacity = 100, iterable = False):
        ...
        for _name, _shape, _dtype, _lod_level in zip(self.names, self.shapes, self.dtypes, self.lod_levels):
...
if __name__ == "__main__":
    ...
    mnist_input_slots = [
        {"name":"image", "shape":(-1, 32, 32, 1), "dtype":"int32"},
        {"name":"label", "shape":[-1, 1], "dtype":"int64"}
    ]

    input_field = InputField(mnist_input_slots)
    
    input_field += {"name":"large_image", "shape":(-1, 64, 64, 1), "dtype":"int32"}
    input_field += {"name":"large_color_image", "shape":(-1, 64, 64, 3), "dtype":"int32"}
